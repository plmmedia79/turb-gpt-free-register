...
class SMSBowerWebUiTests(unittest.TestCase):
    ...
    def test_jobs_rejects_smsbower_without_api_key_before_creating_tasks(self, submit_registration):
        submit_registration.return_value = []
        with patch.object(email_config, "USE_EMAIL_SERVICE", True), patch.object(
            email_config, "EMAIL_SOURCE", "smsbower"
        ), patch.object(email_config, "SMSBOWER_API_KEY", ""):
            response = self.client.post("/api/jobs", json={"count": 1, "workers": 1})

        self.assertEqual(response.status_code, 400)
        self.assertIn("请填写 SMSBower API Key", response.get_json()["error"])
    ...
    def test_jobs_with_smsbower_key_does_not_check_outlook_pool(self, submit_registration, outlook_pool_summary):
        outlook_pool_summary.return_value = {"total": 0, "available": 0, "used": 0, "failed": 0}
        with patch.object(email_config, "USE_EMAIL_SERVICE", True), patch.object(
            email_config, "EMAIL_SOURCE", "smsbower"
        ), patch.object(email_config, "SMSBOWER_API_KEY", "key-123"):
            response = self.client.post("/api/jobs", json={"count": 1, "workers": 1})
    ...
    def test_summary_does_not_count_smsbower_as_outlook_pool(self, count_accounts, outlook_pool_summary, domain_pool_summary):
        outlook_pool_summary.return_value = {"total": 0, "available": 0, "used": 0, "failed": 0}
        with patch.object(email_config, "EMAIL_SOURCE", "smsbower"):
